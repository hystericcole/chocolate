...
extension CHCLT {
	...
	public func interpolate(colorLocations:[CHCLTShading.ColorLocation], by scalar:Linear) -> Vector4 {
	...
	/// Adjust the luminence of the given color to generate a contrasting color.
	/// # Contrast
	/// - 0 : low contrast, medium luminance color.
	...
	public func shiftHexagonal(_ rgb:Vector4, by shift:Scalar) -> Vector4 {
		return hexagonal(hue:RGBA.hsb(rgb).hue + shift, saturation:saturation(rgb), luma:luma(rgb), alpha:rgb.w)
	}
...
public struct CHCLTShading {
	public struct ColorLocation {
		...
		public let location:CHCLT.Linear
	}
	...
	public init(model:CHCLT, colors:[CHCLT.Vector4]) {
		let scalar = CHCLT.Linear(colors.count - 1)
		
		self.init(model:model, colors:colors.indices.map { ColorLocation(color:colors[$0], location:CHCLT.Linear($0) / scalar) })
	}
	
	public init(model:CHCLT, colors:[CHCLT.Vector4], locations:[CHCLT.Linear]) {
		let indices = 0 ..< min(colors.count, locations.count)
		
		self.init(model:model, colors:indices.map { ColorLocation(color:colors[$0], location:locations[$0]) })
	}
	...
	public func shadingFunction() -> CGFunction? {
		...
		let evaluate:CGFunctionEvaluateCallback = { pointer, input, output in
			guard let linearGradient = pointer?.assumingMemoryBound(to:CHCLTShading.self).pointee else { return }
			
			let rgba = linearGradient.model.interpolate(colorLocations:linearGradient.colors, by:CHCLTScalar(input.pointee))
